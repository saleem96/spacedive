@extends('dashboard',['a' => __('strings.new_invoice')])
@section('body')
            ...
            <div class="flex flex--wrap flex--justify-space-between m-v-md  m-v-xl--desktop" id="invoice__buttons">
                ...
                <a href="javascript:window.print()" >
                    <div class="button button--blue-inverted m-v-sm">
                        ...
                        <span style="color: #5354CE">{{__('strings.print')}}</span>
                    </div>
                </a>
                ...
                <a href="javascript:window.print()" >
                    <div class="button button--blue-inverted m-v-sm">
                        <!--                <img src="img/icon_file_sm.svg" width="10" height="10" alt="file" class="m-r-sm">-->
                        <span style="color: #5354CE">{{__('strings.preview')}}</span>
                    </div>
                </a>
                    ...
                    <select name="client_id" class="" id="clients">
                        ...
                        @foreach($clients as $client)
                            ...
                            <option value="{{$client->id}}"
                                    ...
                                    @if(isset($client_id) && $client_id == $client->id)
                                        selected
                                    @endif
                            >{{$client->name}}</option>
                        @endforeach
                ...
                <button form="invoice__delete" class="invoice__deleteImageFlip button button--blue-inverted button--red-inverted-hover m-v-sm">
                    ...
                    <span style="color: #5354CE">{{__('strings.delete')}}</span>
                </button>
            </div>
...
{{--
                    ...
                    <div class="m-v-md ">
                        <span>Ref.</span>
                        <input type="text" id="reference_no"  value="{{isset($task) ? $task->reference_no:''}}"placeholder="Reference number" class="">
                    </div>
                    <div class="m-v-md ">
                        <input type="text" name="invoice_num" placeholder="" value="{{__('strings.invoice')}} {{\App\Invoice::where('user_id',auth()->id())->count() + 1}}" class="font-size--header blue ">
                    </div>


                    <div>
                        <input id="invoice__description" name="description" class="invoice__description invoice__description--input inputBorder p-sm ellipsis" type="text"   data-task="name" value="{{isset($task) ? $task->name:''}}" placeholder="{{__('strings.project_desc')}}" maxlength="255" autocomplete="off"
                ...
                <div class="invoice__dates m-v-lg">
                    ...
                    <div class="m-v-md">
                        ...
                        <div class="gray m-r-sm display--inline--tablet">{{__('strings.issued')}}</div>
                        <input id="invoice__dateIssued" class="datepicker" name="date_issued" type="text" placeholder="yyyy-mm-dd" value="{{date('Y-m-d')}}" autocomplete="off" required>

                    </div>


                    <div class="m-v-md">
                        <div class="gray m-r-sm display--inline--tablet">{{__('strings.due')}}</div>
                        <input id="invoice__dateDue" class="datepicker" required name="date_due" type="text" placeholder="{{__('strings.yyyy')}}"   value="{{date('Y-m-d')}}"  autocomplete="off" >
                    </div>
                ...
                <h4 style="color: #4E4E4E;margin-bottom: 10px">{{__('strings.who')}}</h4>

                <div class="flex flex--start">

                    <div class="m-v-md ">

                        <input type="text" id="name" name="client" placeholder="{{__('strings.company_name')}}" class="">
                    </div>
                </div>
                <div class="flex flex--start">
...
{{--                        <label for="">Registration Number</label>--}}
                        ...
                        <input type="text" id="num" name="reg_num" placeholder="{{__('strings.registration_num')}}" class="">
                    </div>
                </div>
                <div class="flex flex--start">
...
{{--                        <label for="">Registration Number</label>--}}
                        ...
                        <input type="text" id="ean" name="ean" placeholder="{{__('strings.ean')}}" class="">
                    </div>
                </div>
                <div class="flex flex--start">
...
{{--                        <label for="">Billing Address</label>--}}
                        ...
                        <input type="text" id="address" name="address" placeholder="{{__('strings.billing_address')}}" class="">
                    </div>
                </div>
                <div class="flex flex--start">
...
{{--                        <label for="">Contact Name</label>--}}
                        ...
                        <input type="text" id="cname" name="contact" placeholder="{{__('strings.contact_name')}}" class="">
                    </div>
                </div>
                <div class="flex flex--start">
...
{{--                        <label for="">Company Email</label>--}}
                        ...
                        <input type="text" id="email" name="email" placeholder="{{__('strings.email')}}" class="">
                    </div>
                </div>
                <div class="flex flex--start">
...
{{--                        <label for="">Company Phone</label>--}}
                        ...
                        <input type="text" id="phone" name="phone" placeholder="{{__('strings.phone')}}" class="">
                    </div>
                </div>
                ...
                <ul class="invoice__expenses display--table fullWidth fullWidth--max m-t-lg m-t-xl--tablet">
                    <li class="gray display--table-row">
                        <span class="expense__colDesc display--table-cell left">{{__('strings.desc')}}</span>
                        <span class="expense__colQty display--table-cell right">{{__('strings.qty')}}.</span>
                        <span class="expense__colPrice display--table-cell right">{{__('strings.price')}}</span><span
                            class="expense__colTotal display--table-cell right">{{__('strings.total')}}</span>
                    </li>
                </ul>
                ...
                <div class="m-v-md ">
                    ...
                    <textarea id="invoice__notes" name="notes" class="textarea--v p-sm borderBox fullWidth" placeholder="{{__('strings.notes')}}..." autocomplete="off" maxlength="2000" oninput="autosizeTextarea.bind(this, event)();"></textarea>
                  ...
                  <div class="row">
                                <div class="col-lg-4">
                                    <div class="form-group mb-3">
                                        <label class="form-control-label" for="input-first-name"> {{__('strings.holiday')}}</label>
                                        <div class="form-check well">
                                            ...
                                            <label class="form-check-label" for="exampleCheck1"> {{__('strings.pay_holiday')}} </label>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <span>
                <label for="invoice__currency" class="gray m-r-sm">{{__('strings.currency')}}</label>
                <select id="invoice__currency" name="currency" class="m-r-md">
                    ...
                    <div class="details__column flex__basis0 flex__grow flex flex--baseline invoice__submitContainer">
                        ...
                        <div class="flex__grow"></div>
                        <button  name="submit" value="draft"   class="button button--blue-inverted m-v-sm invoice_submit_button" type="submit" style="margin-right:10px;color:#5354CE">{{__('strings.save_draft')}}</button>
                        <button id="invoice__submit" class="button button--blue right invoice_submit_button"  name="submit" value="send" type="submit" style="background-color: #5354CE !important;">{{__('strings.send_spacedive')}}</button>
                    </div>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title">{{__('strings.select')}} Gig</h5>
                        </div>
                        <form action="" method="get">
                            ...
                            <div class="container">
                                <select name="task_id" id="" class="form-control">
                                    @foreach($tasks as $item)
                                        <option value="{{$item->id}}">{{$item->name}} / {{ $item->client->name }} / {{ \Carbon\Carbon::parse($item->created_at)->format('d/m/Y')}}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="modal-footer">
                                <button type="submit" class="btn btn-primary">{{__('strings.select_gig')}}</button>
                            </div>
                        </form>
...
@section('script')
    <script>
        ...
        $("#invoice__submit").click(function(e) {
            if(!confirm("{{__('strings.sendspacedive_msg')}}")) {
                e.stopPropagation();
                return false;
            }
        });
